...
- 데스크톱의 원문 편집기와 미리보기는 같은 폭과 같은 작업 높이를 사용한다. 세로 스크롤은 서로 다른 렌더 높이를 고려해 픽셀이 아니라 전체 문서 진행률로 양방향 연동하며, 독립적으로 비교하려는 사용자는 `스크롤 연동`을 끌 수 있다. 모바일에서는 두 영역을 세로로 쌓되 같은 작업 높이 강제는 적용하지 않는다.
