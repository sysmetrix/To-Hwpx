...
- 일반 사용자 동선에서는 직접 입력, 업데이트 내역 상세, 추천 실험 기능을 숨긴다. `?admin=1` 또는 호환용 `?lab=1`로 관리자 모드에 들어가면 localStorage에 상태를 저장한다.
...
- 포맷 품질 평가는 버전/일자별 추이를 보여주되, 원격 통계처럼 표현하지 않는다. 현재 값은 `FORMAT_QUALITY_METRICS`와 `QUALITY_HISTORY`에서 관리하며 `FORMAT_INFO.limits`를 함께 보여준다.
...
검증:
- 직접 입력 미리보기 회귀는 Markdown 제목·문단·표가 미리보기 영역에 표시되는지 확인한다.
