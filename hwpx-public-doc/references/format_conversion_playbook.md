...
- **직접 입력**은 v4.8.3부터 일반 사용자에게 베타로 공개되었다. 탭은 항상 노출되고, 탭 진입 시 품질 안내 패널이 표시된다. 업데이트 내역 상세, 구현된 실험 기능 목록, 추천 실험 기능, 포맷 품질 평가, 미리보기 패널(`paste_preview`), HTML 복사·다운로드 메뉴(`html_actions`)는 관리자 모드에서만 노출된다. `?admin=1` 또는 호환용 `?lab=1`로 관리자 모드에 들어가면 localStorage에 상태를 저장한다.
