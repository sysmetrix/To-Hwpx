...
관련 코드: `js/pdf-parser.js`(조립) · `js/pdf-style.js`(서식) · `js/pdf-graphics.js`(그림·괘선) · `js/pdf-table.js`(격자) · `js/pdf-layout.js`(머리말·다단)
게이트: `qa/pdf-gate.js`(`npm run test:pdf`) · `qa/pdf-fidelity-gate.js`(`npm run test:pdf-fidelity`)

### 전제 — PDF는 구조를 담지 않는다

PDF는 **레이아웃 기술 형식**이다. 문단·표·목록 같은 논리 구조가 아니라 "이 글자를 이 좌표에 이 크기로 그려라"만 담는다. 따라서 구조 복원은 본질적으로 **추론**이며, 다른 포맷처럼 "원본과 같은가"를 물을 수 없다.

이 사실을 숨기지 않는다. 추론에 쓴 근거를 `ir.audit`에 남기고 결과 카드·CLI·MCP가 그대로 보여준다. **고정 백분율로 품질을 표기하지 않는다.**

### 추론 규칙

| 대상 | 근거 |
|---|---|
| 본문 글자 크기 | 글자 높이의 최빈값(글자 수로 가중) |
| 제목 | 본문보다 15% 이상 큰 줄. 크기 내림차순으로 H1~H6 |
| 표 | 열 경계가 맞는 줄이 **2줄 이상** 연속 |
| 머리행 | 글자가 더 크거나 **글꼴 식별자가 아래 행과 다를 때만** |
| 목록 | 본문 왼쪽 여백보다 안쪽에서 시작하는 줄 |
| 목록 중첩 | 문서에서 관측한 최소 들여쓰기를 한 단계로 |
| 문단 이어짐 | 줄 간격이 줄 높이의 1.9배 이내이고 글자 크기가 같을 때 |

### 근거 없이 주장하지 않는다

**머리행 판정이 이 원칙의 대표 사례다.** 표의 첫 행을 머리행으로 만드는 것은 대개 맞지만, 근거가 없으면 주장하지 않는다.

- 글꼴이 다르다(`th`가 굵은 변형으로 그려짐) → 머리행으로 지정
- 글자 크기가 더 크다 → 머리행으로 지정
- 둘 다 없다 → **그냥 행으로 둔다**

`qa/pdf-gate.js`의 `headerRow: false` 기대값은 "머리행이 아니어야 한다"가 아니라 **"근거가 없으므로 주장하면 안 된다"**를 검사한다.

한 줄짜리 "표처럼 보이는 줄"도 표로 만들지 않는다. 들여쓴 문단이나 좌우 정렬된 머리말이 표로 둔갑하면 원문보다 나쁜 결과가 된다.

### 실제로 밟은 함정

1. **머리행은 가운데 정렬되어 x 좌표가 본문 행과 어긋난다.** 좌표만 보면 머리행이 표에서 떨어져 나가 "들여쓴 줄" — 즉 **목록으로 오분류**된다. 열 *개수*가 같고 줄 간격이 표 안 행 간격과 비슷하면 머리행으로 끌어들인다.
2. **들여쓰기 한 단계 폭에 고정 상수를 쓰면 안 된다.** 문서마다 어긋나 1단계 목록이 레벨 1로 잡힌다. 관측된 최소 들여쓰기를 한 단계로 삼는다.
3. **줄·열 판정 기준을 고정 px로 두면 글꼴 크기에 따라 무너진다.** 모두 글자 높이에 비례시킨다.

### 스캔 PDF는 거절한다

글자 레이어가 없는 PDF(스캔 이미지)는 추출할 것이 없다. **빈 HWPX를 성공으로 돌려주면 사용자는 변환된 줄 안다.** 그래서 명시적으로 거절하고 OCR 미지원 사실을 알린다. `qa/pdf-gate.js`가 `blank-scan.pdf`로 이 동작을 고정한다.

### 글자만으로는 알 수 없는 것들 — 연산자 목록을 읽어야 한다

`getTextContent()`는 **글자와 좌표만** 준다. 아래는 전부 `getOperatorList()`에만 있다.

| 얻는 것 | 방법 | 실측으로 확인한 함정 |
|---|---|---|
| 글꼴 원래 이름 | `page.commonObjs.get(fontName).name` | **열거 가능한 속성이 아니다.** `getOwnPropertyNames`에 안 나오니 직접 `.name`으로 읽는다. `getOperatorList()`를 먼저 부르지 않으면 commonObjs가 비어 있다 |
| 글자색 | `setFillRGBColor` | v6는 이미 `'#c00000'` 문자열로 준다. **조각 경계를 넘나든다** — `"그리고 빨간 글자와"`가 검정-빨강-검정인데 한 조각이라, 글리프 단위로 대응시켜야 한다 |
| 그림 | `paintImageXObject` + `objs`/`commonObjs` | `paintJpegXObject`는 **v6에 없다**. `g_` 접두사 id는 `commonObjs`에 있고, 25만 픽셀이 넘는 **큰 그림일수록 그쪽**이다. `has()`가 false여도 없는 게 아니라 아직 안 온 것 — 콜백형 `get(key, cb)`로 기다려야 한다 |
| 괘선 | `constructPath` | 인자는 `[paintOp, [Float32Array], bbox]`. **bbox를 공짜로 준다.** 괘선은 선이 아니라 **얇고 긴 채워진 사각형**으로 온다(실측 `(76,145)→(77,174)` = 폭 1pt) |

**CTM은 스택으로 추적해야 한다.** `transform` 인자 하나만 보면 안 되고 `save`/`restore`로 쌓인 것을 모두 곱해야 한다. 안 하면 90×60pt 그림이 **375×250으로** 나온다(실측).

⚠ `constructPath`의 인자는 pdf.js가 렌더링할 때 **제자리에서 Path2D로 바뀐다.** 렌더링 전에 읽어야 한다.

### ⚠ pdf.js가 주는 `width`는 믿을 수 없다

실제 한글 공문서(양쪽 정렬)에서 실측한 값이다. 11pt 글꼴인데:

```
"AI"          x= 72.6  w=10.4    → 83.0에서 끝남
" "           x= 83.0  w=55.8    ← 공백 하나가 55.8pt?
"행위자는 AI를" x= 89.8            ← 실제 전진은 6.8pt
"⚫"           x= 57.6  w= 0.0    ← 글머리 기호가 폭 0?
```

이 두 값을 그대로 믿었을 때 벌어진 일:

1. **공백 조각을 버리고** 그 엉터리 폭으로 열을 나눴다 → 낱말이 붙고(`AI행위자는`), 한 줄이 여러 열로 쪼개져 **문단이 표로 둔갑**했다. 23쪽 문서에서 **가짜 표 19개**.
2. 글머리 기호 폭이 0이라 뒤 글자와의 간격이 15pt로 계산됐다 → `⚫`이 제 열이 되고, 줄마다 같은 자리에 오니 **불릿 목록 전체가 2열 표**가 됐다.

**그래서 폭은 좌표로 재고, 값이 그럴듯할 때만 믿는다.** 글자 수와 글자 크기로 추정치를 만들어(한글·한자는 글자 크기만큼, 나머지는 절반) 보고된 폭이 그 0.3~2.5배 안일 때만 쓴다. 공백 조각은 "여기 공백이 있었다"는 사실만 기억하고 폭은 좌표로 잰다.

### 표는 괘선이 있으면 추측하지 않는다

공백 클러스터링만 쓰면 열 간격이 좁은 표(`A B C`/`1 2 3`)를 문단으로 만들고, 병합 셀이 있으면 격자가 어긋난다(`사업비 130`이 `120 140 사업비` 순서가 됐다).

괘선이 있으면 선이 곧 격자다. 칸마다 **네 변에 실제로 선이 있는지 관찰**해 병합을 정한다(Camelot `set_span()`과 같은 방식).

- 오른쪽 변이 없다 → 가로 병합(`colSpan`)
- 아래 변이 없다 → 세로 병합(`rowSpan`)

좌표 뭉치기(cluster)에서 **기준을 직전 값으로 잡으면 안 된다.** 3pt씩 떨어진 좌표가 끝없이 이어져 열 전체가 하나로 무너진다. 그룹의 **첫 값**을 기준으로 둔다.

머리행은 괘선이 있어도 **글꼴·크기 근거가 있을 때만** 주장한다(위 원칙 그대로).

#### 쪽 하나에 격자 하나가 아니다

쪽의 괘선을 전부 모아 격자를 **하나만** 세우면, 머리말 밑줄·제목 밑줄·설명 상자·진짜 표가 한 표로 뭉쳐 **14행짜리 가짜 표**가 된다. 제목과 본문 문단이 셀 안에 갇혀 문서를 읽을 수 없다.

pdfplumber 방식으로 나눈다. ① 교차점에서 시작해 ② 오른쪽·아래로 **가장 가까운 닫힌 사각형**을 찾고(네 변이 모두 실제 선이어야 한다) ③ 꼭짓점을 공유하는 셀끼리 묶는다. **묶음 하나가 표 하나**다. 병합 셀은 여러 칸을 덮는 큰 셀로 자연히 나오므로 병합을 따로 추론할 필요가 없다.

#### 테두리는 칸마다 끊겨 그려진다 — 먼저 이어 붙여라

표 테두리가 하나의 긴 선일 거라고 가정하면 안 된다. 실측하면 왼쪽 세로 테두리가 이렇게 온다.
...
이어 붙이지 않으면 **두 행에 걸친 셀(세로 병합)을 절대 못 찾는다.** 어떤 선분 하나도 그 범위를 덮지 못하기 때문이다. 실제로 병합 셀과 머리행이 통째로 사라졌다. 같은 직선 위 선분을 3pt 틈까지 이어 붙인 뒤에 덮임을 판정한다(pdfplumber `join_edge_group`).

#### 채우기와 획을 다르게 다뤄야 한다

- **채우기(fill)**로 그린 괘선은 *얇고 긴 사각형*이라 `constructPath`의 bbox만으로 충분하다.
- **획(stroke)**으로 그린 사각형은 `constructPath` 하나에 네 변이 다 들어가고 **bbox가 상자 전체**다. 얇지 않으니 통째로 버려져 **괘선이 하나도 안 잡힌다.** 좌표 스트림을 직접 훑어 변마다 따로 내야 한다.

DrawOPS는 pdf.js가 내보내지 않으므로 숫자로 적을 수밖에 없다(v6 실측): `0=moveTo(2) 1=lineTo(2) 2=curveTo(6) 3=quadraticCurveTo(4) 4=closePath(0)`.

#### 셀 음영과 열 너비도 가져와야 한다

`constructPath`에서 **얇은 것만** 괘선으로 쓰고 나머지를 버리면, 그 버려지는 것 안에 **셀 배경색**이 있다. 실측하면 23쪽 문서에서 채워진 면 208개가 버려지고 있었다.

그 결과가 심각하다. 표 머리행이 **흰 글자 + 배경 없음**이 되어 한글에서 **흰 바탕에 흰 글자**가 된다 — 글자가 아예 안 보인다. 실문서에서 표 20개가 그 상태였다.

셀을 **절반 넘게 덮는** 면만 그 셀의 음영으로 본다. 중심을 포함하는 면을 쓰면 쪽 전체를 덮는 도형이 모든 셀의 배경이 된다. 셀보다 2.5배 넘게 큰 면도 배경 도형이지 셀 음영이 아니다. 흰색(`#ffffff`)은 기본 바탕이라 넣지 않는다 — borderFill만 늘어난다.

**불변식: 흰 글자 셀에는 반드시 배경이 있어야 한다.** 없으면 글자가 사라진 것과 같다.

**열 너비는 괘선 간격에서 온다.** 글자 길이로 추정하면 `주체`처럼 짧은 머리글이 든 좁은 열이 넓어져 원본과 달라진다. `columnWidthsHwp`에 실제 x 경계 간격을 넣는다(1pt = 100 HWP 단위).

#### 테두리만 있는 상자는 표가 아니다

안에 세로 칸막이가 없는 영역은 **설명 상자**다. 표로 만들면 본문 문단이 셀에 갇혀 읽기 흐름이 끊긴다. 열이 하나뿐인 묶음은 표로 만들지 않고 본문으로 흘려보낸다.
...
들여쓰기만 보고 새 항목으로 만들면 **한 항목이 두 개로 쪼개진다.** 그래서 들여쓰기 판정보다 **이어짐 판정을 먼저** 하고, 항목 첫 줄(`startX`)보다 더 들여쓴 줄은 이어지는 줄로 본다. 같은 x에서 시작하면 새 항목이다(번호 목록 `1.`/`2.`가 그렇다).

### 글머리 기호는 열이 아니라 목록이다

`⚫`·`·`·`･`가 뒤 글자와 떨어져 있으면 **열 경계가 아니라 목록 표시**다. 구분하지 못하면 불릿 문단이 통째로 2열 표가 된다. 줄 맨 앞의 기호는 떼어내 목록 항목으로 만들고, 뗀 기호는 `audit.convertedMarkers`에 적는다 — **잃어버린 것이 아니라 구조로 바뀐 것**이라는 뜻이다.

글머리 기호로 시작하는 줄은 **언제나 새 항목**이다. 이어 붙이면 여러 항목이 한 문단으로 뭉친다.

### 다단은 줄을 묶기 **전에** 갈라야 한다

`assembleLines`는 y가 같으면 쪽 전체를 가로질러 한 줄로 묶는다. 그래서 2단 편집에서는 **왼쪽 단의 줄과 오른쪽 단의 줄이 같은 줄 객체가 된다**(실측: `"오른쪽 단으로 넘어간 문장입니다. 왼쪽 단을"`). 이미 합쳐진 뒤에 순서만 바꿔서는 고칠 수 없다.

글자 조각 점유도로 골짜기를 찾아 **합쳐진 줄을 도로 가른 뒤** 순서를 잡는다. 재귀 XY-cut을 그대로 쓰지 않는 이유는 전폭 제목 하나만 있어도 세로 컷이 불가능해져 2단이 1단으로 무너지기 때문이다 — 전폭 줄을 먼저 밴드로 떼어낸다.

두 가지가 딸려 온다.
- **들여쓰기는 그 단의 왼쪽 끝 기준으로 잰다.** 쪽 왼쪽 기준이면 오른쪽 단 전체가 목록이 된다.
- **단이 바뀌면 문단을 잇지 않는다.** 왼쪽 단 맨 아래 → 오른쪽 단 맨 위는 y가 거꾸로 커져 간격이 음수가 되고, 그대로 두면 "아주 가까운 줄"로 읽혀 두 단의 문장이 한 문단이 된다.

### 머리말·꼬리말은 지우되, 지웠다고 적는다

쪽마다 반복되는 머리말·쪽번호가 본문에 섞이면 10쪽 문서에서 같은 문장이 10번 박힌다. 세 조건을 **모두** 만족할 때만 뺀다.

① 쪽 위/아래 가장자리 띠(12%) 안 · ② 여러 쪽에서 거의 같은 높이 · ③ 숫자를 뺀 글자가 같음(`- 1 -`과 `- 2 -`)

**뺀 것은 `audit.removedPageFurniture`에 남긴다.** 그래야 "일부러 뺀 것"과 "잃어버린 것"이 구분된다. 문자 보존의 기준은 *아무것도 안 사라짐*이 아니라 **조용히 사라진 것이 없음**이다.

### 한글 PDF는 CMap이 있어야 한다

한국어 PDF는 흔히 미리 정의된 CMap(`UniKS-UCS2-H`, `KSCms-UHC-H`, Adobe-Korea1 계열)을 쓰는 Type0/CIDFont로 만들어진다. ToUnicode가 없으면 pdf.js는 그 `.bcmap` 파일을 읽어야 유니코드를 복원한다. 없으면 **조용히 깨진 글자나 빈 문자열**이 나오고, 증상은 "어떤 PDF는 되고 어떤 건 안 된다"로 보인다.

`js/vendor/pdfjs-6.3.289/cmaps/`에 169개를 함께 두고 `cMapUrl`(끝 슬래시 필수 — 없으면 pdf.js가 예외를 던진다) + `cMapPacked: true`로 넘긴다. 요청은 self-origin이라 CSP를 통과하고, 서비스 워커가 필요할 때만 캐시한다.

### 한글 줄바꿈 공백 — 세어 보고 정한 규칙

줄이 바뀔 때 PDF는 그 자리의 공백을 **버린다.** 조각 문자열에도 좌표에도 남지 않는다(양쪽 정렬이라 모든 줄이 오른쪽 끝에 딱 붙어 여백으로도 구별되지 않는다). 그래서 글자만 보고 판단해야 한다.

한동안은 "근거가 없다"며 공백을 아예 넣지 않았다. **그런데 세어 보니 그게 90% 틀린 선택이었다.** 한글 공문서 23쪽에서 이어지는 줄 46쌍을 뽑아 분류한 결과:

| 유형 | 예 | 비율 |
|---|---|---|
| 어절 경계(공백 필요) | `향상과`+`새로운`, `기여할`+`수` | **약 90%** |
| 어절 중간(공백 없음) | `차원`+`에서는`, `존`+`중하고` | 약 10% |

그래서 **기본은 공백을 넣고**, 아래 두 가지에 걸릴 때만 붙인다.

1. **앞 줄이 홀로 못 쓰는 한 글자로 끝날 때** — `…칙을 존`+`중하고` → `존중하고`, `…추진하기 위`+`하여` → `위하여`. `수`·`것`·`등`처럼 홀로 쓰이는 글자는 제외한다(`기여할 수` + `있다`).
2. **뒤 줄이 홀로 못 쓰는 조사·어미로 시작할 때** — `…인류 차원`+`에서는` → `차원에서는`.

`하`·`되` 계열 어미는 목록으로 적으면 끝이 없어(`할지에`, `하도록`, `되므로`…) 규칙으로 둔다. **두 번째 글자까지 함께 봐야 안전하다** — `하나의`·`함께`·`한국`·`해결`은 어엿한 낱말이라 걸리면 안 된다.

예외 하나: 앞 어절이 보조적 연결어미(`-아야/어야/게/지`)로 끝나면 뒤의 `한다`·`된다`는 어미가 아니라 **보조용언**이다. `흘러야 한다`를 `흘러야한다`로 붙이면 안 된다.

관형사(`본`·`각`·`매`·`총`)는 한 글자여도 뒤 낱말과 띄어 쓴다. `본 윤리원칙`을 `본윤리원칙`으로 붙이면 안 되므로 "홀로 쓰이는 한 글자" 목록에 넣는다.

**⚠ 문장부호 판정은 언어 판정보다 먼저 해야 한다.** 한동안 "앞이 한글로 끝날 때만" 한글 규칙을 태웠는데, 그러면 `보상,`+`일자리의`나 `한다.`+`나아가`가 라틴 규칙으로 떨어진다. 라틴 규칙은 뒤가 영문·숫자일 때만 공백을 넣으므로 한글이 오면 **그냥 붙어 버린다**(`한다.나아가`). 실문서 58쌍 중 6쌍이 이 경로로 틀렸다.

가운뎃점(`·`)으로 끝나면 이어지는 표기다(`공정성·포용성`). 다른 문장부호로 끝나면 어절 경계가 확실하다.

#### 문서를 사전으로 쓸 수 있을까 — 안 된다

"뒤 조각이 문서 어디에도 낱말로 안 나오면 조각이다"라는 생각을 실측해 봤다. 23쪽 문서에서 이어지는 줄 58쌍 중 뒤 조각이 사전(줄 안에서만 나온 어절 1,729개)에 **있는 것 30개, 없는 것 28개**였다. `데에도`·`자율성의`·`일자리의`처럼 **멀쩡한 낱말이 한 번만 나와서** 사전에 없는 경우가 절반이라 신호가 되지 못한다. 규칙 기반이 낫다.

#### 규칙은 표로 검사한다

규칙이 여러 겹이라 픽스처가 실패해도 **어느 규칙이 깨졌는지** 알 수 없다. 그래서 `npm run test:korean-join`이 규칙 자체를 표로 검사한다(25건, 전부 실문서에서 관찰한 사례). 문장부호 우선 판정을 되돌리면 4건, 관형사를 빼면 2건이 실패한다.

**남은 한계**: 어절 중간에서 끊겼는데 뒤 줄이 조사·어미가 아닌 경우(`존`+`중하고` 같은 형태인데 앞이 두 글자 이상일 때)는 여전히 공백이 들어간다. 사전 없이는 판별할 수 없다.

### 보존 / 제외 가능

- 보존: 본문 글자와 문단, 추론한 제목 단계, **괘선 표(가로·세로 병합·셀 음영·열 너비 포함)**, 열이 맞는 표, 들여쓴 목록과 중첩 단계, **굵게·기울임·글자색**, **본문 그림**, **2단 편집 읽기 순서**.
- 제외: 중첩 표, 글꼴 종류(굵게·기울임은 보존하되 글꼴 이름 자체는 옮기지 않음), 페이지 배치, **표 밖의 도형·벡터 그래픽**(표지 그라데이션 막대, 장식 상자 등 — 괘선과 셀 음영만 가져온다), 테두리 색·굵기, 스캔 PDF의 글자(OCR 미지원).
- 뺀 것: 쪽마다 반복되는 머리말·꼬리말·쪽번호 — 본문이 아니라 쪽 장식이므로 제외하고, 무엇을 뺐는지 `audit`에 적는다.
- 안내: 원본 문서 파일(DOCX·HWP 등)이 있으면 그쪽이 항상 더 정확하다. PDF는 원본이 없을 때 쓰는 경로다.

### 검증

`npm run test:pdf` — `tests/make-pdf-fixtures.js`가 **구조를 우리가 아는 HTML**에서 PDF를 만들고, `qa/pdf-gate.js`가 기대값과 대조한다. 픽스처 HTML과 기대값은 짝이므로 한쪽을 고치면 다른 쪽도 고친다.

`npm run test:core`에도 `sample.pdf`를 넣어 웹앱과 CLI 산출물이 같은지 확인한다(현재 바이트 동일).

`npm run test:pdf-fidelity` — **일부러 어려운 케이스 8종**(서식·그림·병합셀·좁은 괘선표·2단·하이픈/합자·반복 머리말·한글 공문서)으로 세 가지를 **따로** 잰다.

| 지표 | 무엇을 보나 | 기준 |
|---|---|---|
| 문자 보존 | 글자가 사라졌는가(순서 무시) | **하한선.** 일부러 뺀 쪽 장식·글머리 기호는 `audit`과 대조해 제외 |
| 읽기 순서 | 글자가 읽는 순서대로 있는가 | 2단·병합셀에서 글자는 다 남고 순서만 뒤섞이는 일이 실제로 있다 |
| 구조 정확도 | 제목·표·목록·그림·서식이 정답과 맞는가 | 단언 개수 대비 통과 수 |

#### 지표 자체가 틀릴 수 있다 — 실제로 두 번 틀렸다

측정 코드는 조용히 거짓말을 한다. 실문서로 재 보고서야 드러났다.

1. **`irText()`가 `text`와 `runs`를 둘 다 세었다.** 둘은 같은 내용이라 서식 있는 블록마다 글자가 두 번 들어갔다(`서문서문`). 보존율은 부풀고 순서 비교는 통째로 어긋났다. 서식이 있으면 `runs`가, 없으면 `text`가 정본이다.

2. **읽기 순서를 탐욕적으로 맞췄다.** 앞에서부터 같은 글자를 찾다 보니 쪽번호 `1`이 본문 `1.1.`의 `1`에 걸려 커서가 `서문`을 지나쳐 버렸다. 멀쩡한 23쪽 문서가 **순서 2%**로 나왔다. 지금은 **띠(band) 안에서 최장 공통 부분수열(LCS)**로 잰다 — 두 문자열이 거의 같으므로 정렬이 대각선을 벗어나지 않아 ±W만 계산하면 된다(온전한 LCS는 19,000² = 3억 6천만 칸이라 못 쓴다).

"일부러 뺀 글자"를 원문에서 **문자열 검색으로 지우는 것도 위험하다.** 쪽번호 `1`을 지우면 `2026.8.21.`의 첫 `1`이 지워진다. 지우지 말고 **분모에서 빼라.**

**셋을 하나로 뭉치지 않는다.** 뭉치면 글자를 잃고도 구조 점수로 가릴 수 있다. 쉬운 문서에서 나온 100%는 PDF 변환이 100%라는 뜻이 아니므로 픽스처는 어려운 것만 모았다.

⚠ 추론은 문서마다 다르게 맞는다. 이 게이트는 **회귀 감지용이지 품질 보증이 아니다.**
...
- XLSX는 CVE-2023-30533/CVE-2024-22363 수정 버전인 SheetJS 0.20.3 고정 파일을 사용한다. 이 약 952KB 라이브러리는 `index.html`의 메인 스레드에서 미리 읽지 않고, XLS/XLSX 파일을 실제로 처리할 때만 `js/xlsx-worker.js`의 Web Worker가 불러온다. 작업자는 첫 시트를 CSV로 바꾼 뒤 CSV 파서를 재사용해 악성 입력이 UI 스레드를 장시간 막지 않게 한다.
...
관련 코드: `js/gov-doc.js` · 게이트: `qa/gov-doc-gate.js` (`npm run test:gov`)

### 근거

추측이 아니라 **규정에 근거한 변환**이다. 「행정업무의 운영 및 혁신에 관한 규정」과 행정안전부 『행정업무 운영 편람』이 정하는 바:

| 항목 | 규정 |
|---|---|
| 기호 순서 | `1.` → `가.` → `1)` → `가)` → `(1)` → `(가)` → `①` → `㉮` |
| 들여쓰기 | 첫째 항목은 **왼쪽 기본선**, 둘째부터 **바로 위 항목 위치에서 오른쪽으로 2타씩** |
| 띄어쓰기 | 기호와 내용 사이 **1타** |
| 줄바꿈 | 둘째 줄부터 **항목 내용의 첫 글자에 맞춤**(원칙) |

### 깊이는 고정표가 아니라 상대 관계다

규정이 정하는 것은 "1.이면 0단계"가 아니라 **"바로 위 항목보다 2타 안쪽"**이다. 그래서 기호 종류 → 실제 단계 대응을 문서를 읽어가며 만든다.

- `1.` 없이 `가.`부터 시작하는 문서도 0단계에서 시작한다.
- 같은 기호가 다시 나오면 그 단계로 돌아가고 아래 단계는 버린다.
- 중간 기호를 건너뛴 문서(`가.` 다음 바로 `(1)`)도 `(1)`이 `가.`보다 **한 단계만** 안쪽에 놓인다. 고정표를 쓰면 `(1)`이 5단계로 튀어 읽는 사람에게 부모 없는 항목처럼 보인다.

### HWPX 좌표 계산 — 실제로 틀렸던 부분

HWPX에서 문단 `left`는 **둘째 줄부터**의 시작 위치이고, 첫 줄은 `intent`(음수)만큼 되돌아가 기호를 놓는다.

```
기호 위치 = left + intent        (intent는 음수)
```
...
이어야 한다. 처음에 `left = (단계+1) × 2타`로 두었더니 **기호 폭이 다른 단계에서 어긋났다** — `1.`과 `1)`은 우연히 맞고 `가.` `가)` `(1)`은 규정보다 앞으로 밀렸다. 우연히 맞는 값이 섞여 있어 눈으로는 잡기 어렵다. 게이트 ④가 8종 전부를 계산으로 대조한다.

기호 폭은 한글·원문자를 1칸, 영숫자·괄호를 0.5칸으로 센다.

### 적용 범위 — Markdown에는 쓰지 않는다

Markdown은 `1. `을 **순서 목록 문법**으로 해석하므로, 파서가 이미 목록 블록을 만든다. 그 위에 공문서 들여쓰기를 얹으면 두 체계가 싸운다.

이 기능은 **문단으로 들어오는 입력**을 위한 것이다 — TXT, PDF, HWP, 직접 입력. 그런 파서는 여러 줄을 한 문단으로 묶어 넘기므로 `applyGovDocStructure`가 **줄 단위로 나눈다.** 기호 없는 줄은 앞 항목의 이어지는 줄이므로 합쳐 둔다(따로 떼면 본문이 조각나고 들여쓰기도 어긋난다).

### 오작동을 막는 것이 기능의 절반이다

기본값은 **끔**이다. 일반 문서에도 "1."로 시작하는 줄은 흔하고, 그것을 전부 공문서 항목으로 취급하면 원문보다 나빠진다.

인식 규칙도 좁게 잡는다.

- 기호 뒤에 **공백이 있어야** 한다(1타 규칙). `1.5배 증가`는 항목이 아니다.
- 숫자는 1~2자리만 본다. `2026. 9. 2. 시행`의 연도를 항목으로 잡지 않는다.
- 기호가 하나도 없는 문단은 **줄 단위로 나누지도 않는다.** 일반 문단을 쪼개면 원문보다 나빠진다.

게이트 ⑦이 "기호 없는 문서를 전혀 건드리지 않는지"를 블록 전체 비교로 확인한다.

### 공문 구성요소 인식

「행정업무의 운영 및 혁신에 관한 규정」 시행규칙이 정하는 구성:

| 구역 | 요소 |
|---|---|
| 두문 | 행정기관명, **수신**, **경유** |
| 본문 | **제목**, 내용, **붙임** |
| 결문 | **발신명의**, **시행**·**접수** 등록번호와 날짜, **공개 구분** |

표지가 **줄 맨 앞**에 있을 때만 인정한다. 본문 가운데 "제목"이라는 낱말이 나온다고 제목 줄로 보면 안 된다.

실제로 바꾸는 것은 최소한이다.

- **제목** → 문서 제목으로 승격하고 본문에서는 제목 문단으로 만든다. 공문에서 제목은 문서를 대표하는 유일한 줄이라 이건 안전하다. 파서는 첫 줄(기관명)을 제목으로 잡아 두는 경우가 많다.
- **발신명의** → 가운데 정렬. 규정 서식에서 결문의 발신명의는 가운데에 온다. `끝.` **이후**에서만 찾는다 — 본문에도 기관장 이름은 흔히 나온다.
- 나머지 → **역할만 표시하고 모양은 두지 않는다.** 이 줄들의 배치는 기관 서식마다 다르고, 추측해서 옮기면 원문보다 나빠진다.

#### 줄 경계는 항목 기호만으로 정하지 않는다

항목 기호만 경계로 삼으면 두문(기관명·수신·경유·제목)이 통째로 한 문단이 되어 **제목을 찾지 못한다** — 실제로 그랬다. 구성요소 표지와 발신명의도 줄 경계다.

### 공문 골격 생성 (`buildOfficialSkeleton`)

MCP `draft_official_document` · 시행규칙의 두문·본문·결문 요소를 순서대로 놓아 초안의 출발점을 만든다.

**공식 별지 서식의 복제가 아니다.** 별지 서식은 기관명 로고 위치, 선 굵기, 결재란 칸 배치까지 정해져 있고, 그것을 추측으로 그리면 "공문서처럼 생겼지만 규격이 아닌 문서"가 나온다 — 원문보다 나쁘다. 그래서 결과에 항상 이 세 줄을 함께 돌려준다.

```
※ 시행규칙이 정한 두문·본문·결문 구성요소를 순서대로 놓은 초안 골격입니다.
※ 공식 별지 서식(기관명 로고 위치, 결재란, 선 굵기 등)의 복제가 아닙니다.
※ 최종 발송본은 소속 기관의 공식 서식 파일에 내용을 옮겨 담아 작성하세요.
```

에이전트가 이 산출물을 발송본으로 오해하면 안 되므로, MCP 테스트가 이 안내의 존재를 검사한다.

#### 붙임 둘째 줄 함정

`붙임  1. 계획서 1부.` 다음의 `      2. 목록 1부.`는 항목 기호처럼 생겼지만 본문 항목이 아니다. 다시 분류하면 붙임에서 떨어져 나와 본문 0단계로 올라간다 — 실제로 그랬다. `classifyLine`은 **역할이 이미 정해진 줄을 다시 분류하지 않는다.**

### 검증

`npm run test:gov` — 기호 8종 인식, 깊이 매김(복귀·건너뜀), HWPX 기호 위치 = 단계 × 2타, 내어쓰기 = 기호 폭 + 1타, 여러 줄 분할, 오작동 방지, 붙임·끝. 인식.

⚠ 구조만 본다. 실제 공문서로 한컴에서 눈으로 확인해야 한다.

## 신구조문대비표

관련 코드: `js/diff-table.js` · 게이트: `qa/diff-table-gate.js` (`npm run test:diff`)

### 지키는 규칙 (법제처 실무 기준)

- **현행을 기준으로 순서를 잡는다.** 개정안 기준으로 정렬하면 현행에서 사라진 조문이 표에서 빠져 "무엇이 없어졌는지"를 볼 수 없다. 그 대신 개정 부분의 위치가 다소 어색해 보일 수 있는데, 이는 실무에서도 그렇다.
- 새로 만든 항목은 현행 칸에 `<신설>`, 없앤 항목은 개정안 칸에 `<삭제>`.
- 바뀌지 않은 항목도 남긴다. 문맥이 없으면 대비표를 읽을 수 없다.
- 바뀐 **쪽만** 굵게. 양쪽을 다 굵게 하면 무엇이 바뀐 건지 안 보인다.

### 틀리면 없느니만 못하다 — 짝짓기가 핵심

대비표에서 서로 관계없는 두 조문이 한 행에 놓이면 읽는 사람은 그것을 "개정됐다"고 읽는다.

**실제로 그런 결함을 만들었다.** 인접한 삭제·신설을 무조건 짝지었더니 `제4조(문서의 보존)`과 `제2조의2(개방형 문서)`가 한 행에 놓였다 — 아무 관계가 없는데 개정된 것처럼 보인다.

고친 방식:

1. **조문 번호가 같으면 같은 조문**이다(본문이 크게 바뀌어도). `제5조` ↔ `제5조`.
2. 한쪽만 조문 번호가 있으면 **다른 것**이다.
3. 번호가 둘 다 없으면 글자 이중자 Dice 유사도 0.5 이상일 때만 짝짓는다.
4. 짝은 **연속된 편집 구간 안에서만** 찾는다. 멀리 떨어진 항목과 묶으면 현행 순서가 무너진다.

게이트 ③이 이 결함을 정확히 재현해 막는다.

### 정렬은 LCS로 한다

순서 비교(i번째끼리)를 쓰면 한 줄만 삽입돼도 그 뒤가 전부 "바뀜"으로 밀린다. 실제로 한 줄 고쳤는데 표 전체가 강조되는 결과가 나온다. 최장 공통 부분 수열로 맞춘 뒤 편집 쌍을 찾는다.

표본이 아주 커서 O(n×m) 표가 메모리를 위협하면 정렬을 포기하고 순서 비교로 내려가되, `report.degraded`로 그 사실을 알린다 — CLI와 MCP가 사용자에게 전달한다.

### 범위

**문단 단위 비교**다. 조·항·호 단위 대비는 법령 구조 파싱이 따로 필요하며, 그것을 문단 비교로 흉내내면 조문 번호가 어긋난 표가 나온다. `제N조` 머리글은 짝짓기 근거로만 쓰고 그 이상은 하지 않는다.

표는 셀을 줄로 펴지 않고 `[표 N행]`으로 요약한다 — 셀을 펴면 대비표가 원문과 전혀 다른 모양이 된다.
...
MCP는 `make_comparison_table`(`current`, `revised`, `changedOnly`).
