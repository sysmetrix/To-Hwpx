...
- 보존: 본문 글자와 문단, 추론한 제목 단계, **괘선 표(가로·세로 병합 포함)**, 열이 맞는 표, 들여쓴 목록과 중첩 단계, **굵게·기울임·글자색**, **본문 그림**, **2단 편집 읽기 순서**.
- 제외: 중첩 표, 글꼴 종류(굵게·기울임은 보존하되 글꼴 이름 자체는 옮기지 않음), 페이지 배치, 도형·벡터 그래픽(괘선 제외), 스캔 PDF의 글자(OCR 미지원).
