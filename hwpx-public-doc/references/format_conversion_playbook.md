...
줄이 바뀔 때 PDF는 그 자리의 공백을 **버린다.** 그런데 한글은 어느 쪽인지 구별할 근거가 없다.

- `…왼쪽 단을` + `모두 읽은…` → 원래 공백이 있었다(어절 경계)
- `…추진하기 위` + `하여 수립…` → 원래 공백이 없었다(어절 중간)

둘 다 같은 Chrome이 만든 PDF이고 줄이 끝나는 x 좌표도 둘 다 오른쪽 여백에 붙어 있어 구별되지 않는다. 공백을 넣으면 `위 하여`, 넣지 않으면 `단을모두` — **손실이 대칭이다.** 근거 없이 고르지 않고, 라틴 문자로 이어질 때만 공백을 넣는다(영문은 어절 중간에서 줄을 바꾸지 않으므로 그때는 근거가 있다). 한글 어절이 붙는 경우가 남는 것은 **알려진 한계**다.
