...
- **표 뒤 간격:** 일반 데이터 표의 `hp:outMargin@bottom`은 `mmToHwp(3)`(850 HWPUNIT, 약 3mm)로 둔다. 이는 표 개체 바깥과 다음 본문 사이의 간격이며 셀 내부 `hp:cellMargin`이 아니다. 페이지 끝에서는 이 공간 때문에 다음 블록이 다음 쪽으로 이동할 수 있으며 정상이다.
