...
## 5. 동시 렌더에서 서식이 일부만 바뀌는 경우

`buildSection()`은 큰 문서에서도 화면이 멈추지 않도록 15개 블록마다 이벤트 루프에 양보한다. 문단·각주·하이퍼링크 ID 카운터나 본문 기준 글자 크기를 모듈 전역에 두면 그 사이에 시작된 다른 변환이 값을 덮어쓴다. 실측에서는 10pt 문서의 20pt run 100개 중 86개가 다른 20pt 문서의 기본 서식(`charPrIDRef="0"`)으로 바뀌었다.

- 카운터·본문 기준 크기·제목 동적 서식은 `buildHwpx()` 호출마다 새 `RenderContext`에 둔다.
- 사전 스캔 결과를 `block._runs`나 `block._cId`로 원본 IR에 쓰지 않는다. 문서별 `WeakMap`에 보관한다.
- `test:core`는 서로 다른 기본 글자 크기의 90문단 문서를 `Promise.all()`로 겹쳐 만들고, 순차 기준과 `header.xml`·`section0.xml`이 같은지 및 입력 IR 불변을 확인한다.
