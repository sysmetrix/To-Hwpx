...
- Markdown 목록 항목은 링크·인라인 서식 보존을 위해 `{text, runs, level, ordered, marker, task, checked}` 객체를 사용할 수 있음.
...
- 목록 항목은 `runs`로 링크와 인라인 서식을 보존함. 표 셀은 현재 문자열 중심이므로 내부 링크·이미지는 표시 텍스트 중심으로 단순화됨.
