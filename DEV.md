...
> **현재 지원 포맷 목록은 이 파일에 따로 적지 않습니다.** 새 포맷이 추가될 때마다 여러 문서를 동기화하는 대신, `js/app.js`의 `SUPPORTED_FORMAT_LABEL`·`FORMAT_INFO`(포맷별 품질·설명의 단일 출처)와 사용자 안내용 [README.md](./README.md) 지원 포맷 표를 기준으로 확인하세요.
...
| [marked.js](https://marked.js.org/) | 18.0.11 | Markdown → HTML 파싱 |
| [SheetJS](https://sheetjs.com/) | 0.20.3 | Web Worker에서 XLSX 파일 읽기 |
| [@rhwp/core](https://github.com/edwardkim/rhwp) | 0.8.4 | HWP5 본문 추출용 JS/WASM |
...
## 상용화 전 최종 마무리 계획 (완료됨 — 역사적 기록)

> 아래는 초기 상용화 준비 단계(v4.0.x 전후)의 스냅샷입니다. 표의 항목은 모두 그 시점에 완료 처리되었고, 이후 릴리스는 [changelog.json](./changelog.json)과 [AGENTS.md](./AGENTS.md)의 릴리스 절차, [qa/release-qa.md](./qa/release-qa.md)로 이어졌습니다. 현재 진행 중인 작업이 아니라 과거 결정의 근거로만 참고하세요.
