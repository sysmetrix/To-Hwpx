...
**직전 화면(v4.21.6):** https://sysmetrix.github.io/To-Hwpx/legacy/v4.21.6/
...
- **직접 입력** — 파일 없이 MD·HTML·TXT·CSV/TSV·JSON 텍스트를 바로 붙여넣어 변환합니다. 형식 추천·오류 진단·미리보기와 HTML 복사·다운로드도 지원합니다.
- **PWA 지원** — 오프라인에서도 동작합니다 (서비스 워커 캐싱).
- **다양한 옵션** — 한글 폰트, 글꼴 크기, 용지 크기, 여백 등을 자유롭게 설정합니다.
- **작업 공간** — 준비 → 설정 → 결과 흐름, 예제 문서, 성공·경고·실패 요약을 한 화면에서 제공합니다.
- **로컬 최근 작업·프리셋** — 파일 메타데이터와 설정만 IndexedDB에 저장합니다. 문서 본문·IR·파일 바이트·Blob·원격 URL은 저장하지 않으며 기록을 끄거나 삭제할 수 있습니다.
- **상용 직접 입력 작업 공간** — MD·HTML·TXT·CSV/TSV·JSON 형식 자동 추천, 줄 번호·찾기/바꾸기, 문법 오류 위치, 대용량 수동 미리보기와 선택형 로컬 초안 복구를 제공합니다. 초안 복구는 기본적으로 꺼져 있고 원문은 서버로 전송하지 않습니다.
