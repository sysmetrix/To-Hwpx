...
- **관리자 전용으로 남은 기능**: 미리보기 패널(`paste_preview`), HTML 복사·다운로드 메뉴(`html_actions`), 업데이트 내역 창, 구현된 실험 기능 목록, 추천 실험 기능, 포맷 품질 평가.
...
- `tests/golden.js`에서 기존 `?lab=1` 호환, MD/HTML/TXT/CSV/JSON 파일 입력과 직접 입력의 HWPX 본문·표·링크·그림 개수 동등성을 검사한다. 직접 입력 탭 노출은 일반 모드에서 확인하고, 미리보기 패널·HTML 메뉴는 관리자 모드에서만 확인한다.
