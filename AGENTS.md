...
**직접 입력(텍스트 붙여넣기 → HWPX)** 은 파일 업로드와 함께 일반 사용자에게 공개한다.

- 지원 형식은 MD, HTML 소스, TXT, CSV/탭 구분 표, JSON이다. 바이너리 형식(DOCX/XLSX/HWP)은 파일 업로드만 사용한다.
- 붙여넣은 문자열을 가상 `File`로 감싸 `fileToIR()` 이후의 기존 변환 파이프라인을 그대로 재사용한다. 별도 간이 파서를 만들지 않는다.
- HTML은 문서형 태그를 우선 보존하며, 태그 없는 일반 텍스트도 문단으로 보존한다. 웹 화면의 CSS 배치 복제를 약속하지 않는다.
- CSV 직접 입력은 쉼표 CSV와 Excel·Google Sheets에서 복사한 탭 구분 표(TSV)를 자동 판별한다.
- 파일 입력과 직접 입력의 동등성은 `tests/golden.js`에서 MD/HTML/TXT/CSV/JSON별 HWPX 본문과 표 개수를 비교한다.
- 직접 입력 지원 범위나 품질이 바뀌면 `FORMAT_INFO`, `getConversionSummaryForExt()`, 안내 문구, 플레이북, QA를 함께 갱신한다.
