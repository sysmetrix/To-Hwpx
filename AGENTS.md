...
DOCX 변환을 변경할 때는 [ARCHITECTURE.md](ARCHITECTURE.md)의 IR v2 경계와 [ENGINEERING.md](ENGINEERING.md)의 하네스를 따른다. 결정 근거는 [docs/adr](docs/adr), 기계 판독 영향도는 [qa/impact-graph.json](qa/impact-graph.json)에 기록한다.

- `js/docx-audit.js`는 파싱 전에 OOXML 결함을 계수하고 의미를 바꾸지 않는 알려진 결함만 복구한다. `blocked` 입력을 추론으로 계속 변환하지 않는다.
- DOCX의 표·행·셀 수는 입력 감사값과 출력 HWPX가 정확히 같아야 한다. 수동 줄바꿈 수는 감소하면 안 되고, 문단 수는 하네스의 명시적 허용치 밖으로 감소하면 안 된다.
- 원본 우선 변환은 페이지 크기·방향·여백, 기본 글자 크기·줄 간격, 표 열 비율, 셀 안의 여러 문단·중첩 블록을 IR에 보존한다. Renderer는 DOCX 확장자를 보지 않는다.
- DOCX 품질을 근거 없는 고정 백분율로 표시하지 않는다. 결과 카드에는 파일별 감사 상태, 복구 내역, 입력→출력 구조 계수, 보존/제외 가능 항목을 표시한다.
- `npm run test:impact`, `npm run test:docx`, 전체 golden/release 게이트를 통과한 뒤 한컴에서 HWPX를 PDF로 내보내 모든 페이지를 contact sheet로 확인한다. 빈 페이지·잘림·누락이 있으면 자동 게이트 통과와 무관하게 릴리스하지 않는다.
- 실제 사용자 원본을 덮어쓰지 않는다. 개선본과 시각 증거는 별도 이름으로 저장한다.

## 🛠 관리자 모드와 실험 기능

완성 전 기능은 지우지 않고 **관리자 모드 뒤에 숨겨** 개발자만 쓴다.

- **직접 입력(텍스트 붙여넣기 → HWPX)** 은 v4.21.0부터 **일반 사용자용 정식 기능**이다. 탭은 항상 노출되며 지원 형식은 MD, HTML 소스, TXT, CSV/탭 구분 표, JSON이다. 바이너리 형식(DOCX/XLSX/HWP)은 파일 업로드만 사용한다. 내용 기반 형식 추천은 사용자의 수동 선택을 덮어쓰지 않으며 `다시 감지`로만 잠금을 해제한다.
- **미리보기 패널(`paste_preview`)과 HTML 복사·다운로드 메뉴(`html_actions`)는 v4.10.6부터 일반 사용자에게 정식 공개**되었다. `ADMIN_FEATURES`에서 제거됐으며 관리자 여부와 무관하게 항상 노출·동작한다.
- **관리자 전용으로 남은 기능**: 업데이트 내역 창, 구현된 실험 기능 목록(현재 비어 있음), 추천 실험 기능, 포맷 품질 평가, **고도화 계획**.
...
- 직접 입력 진단은 `{severity,code,message,line,column,recoverable,action}` 계약을 사용한다. 복구 불가능한 오류는 변환을 막고, 500KB 초과 입력은 자동 미리보기를 중단하며, 직접 입력 자체는 10MB까지 허용한다. 미리보기는 최대 200개 블록만 보여도 HWPX에는 전체 내용을 사용한다.
- 초안 자동복구는 기본 꺼짐이다. 사용자가 명시적으로 켠 경우에만 별도 IndexedDB에 2MB 이하 활성 초안 1개를 최대 7일 저장한다. 복구는 사용자 확인 후 수행하고 기능 해제·초기화·삭제 시 원문을 즉시 지운다. 최근 작업 DB에는 계속 원문을 저장하지 않는다.
- HTML은 태그 없는 일반 텍스트도 문단으로 보존하고, CSV는 쉼표 CSV와 Excel·Google Sheets의 탭 구분 표(TSV)를 자동 판별한다.
- `tests/golden.js`에서 기존 `?lab=1` 호환, MD/HTML/TXT/CSV/JSON 파일 입력과 직접 입력의 HWPX 본문·표·링크·그림 개수 동등성을 검사한다. 직접 입력 탭과 미리보기 패널·HTML 메뉴 노출은 일반 모드에서 확인한다.
...
관리자 모드 `고도화 계획` 탭의 단일 출처는 [roadmap.json](roadmap.json)이다. 화면(`js/roadmap-panel.js`)에는 내용을 적지 않는다 — 갈라지면 계획서를 보고 잘못된 판단을 하게 된다.

- 단계를 끝내면 `roadmap.json`의 해당 항목을 `done`으로 바꾸고 **버전과 PR 번호를 적는다.** 증거 없는 "완료"는 주장일 뿐이다.
- 새 게이트를 만들면 `gates[]`에 추가하고 `test:release`에도 편입한다. 편입하지 않으면 릴리스 때 돌지 않아 계획서만 "검증됨"이라고 말하게 된다.
- `manualVerification[]`은 비우지 않는다. 구조 게이트 통과는 시각 통과가 아니므로 사람이 볼 항목이 항상 남아 있다.
- `npm run test:roadmap`이 위 주장을 저장소에 대조한다. 버전이 changelog에 없거나, 게이트가 package.json에 없거나, test:release에 빠졌거나, 참조 문서가 없으면 실패한다.
