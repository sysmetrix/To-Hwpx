...
- **직접 입력(텍스트 붙여넣기 → HWPX)** 은 v4.8.3부터 **일반 사용자에게 베타로 공개**되었다. 탭은 항상 노출되며, 탭 안에 베타 안내 패널을 표시한다. 지원 형식은 MD, HTML 소스, TXT, CSV/탭 구분 표, JSON이다. 바이너리 형식(DOCX/XLSX/HWP)은 파일 업로드만 사용한다.
