...
const LEGACY_VERSION = '4.20.0';
const LEGACY_COMMIT = '196d117f7587e02867343cf8af615c487b1f890f';
...
assert(archivedIndex.includes(`v${LEGACY_VERSION}`), '고정 커밋의 화면 버전이 v4.20.0이 아님');
...
const stagedLegacySw = archivedSw.replace(
    ...
    "const CACHE_PREFIX = 'to-hwpx-legacy-v4.20.0-';\nconst CACHE_VERSION = 'to-hwpx-legacy-v4.20.0-cache';",
);
assert(stagedLegacySw !== archivedSw, '고정 커밋 서비스워커 패치가 실제 원문과 일치하지 않음');
assert(stagedLegacySw.includes("const CACHE_VERSION = 'to-hwpx-legacy-v4.20.0-cache'"), '레거시 캐시 버전 패치 실패');
