...
const LEGACY_VERSION = '4.19.3';
const LEGACY_COMMIT = '8fcc68338f7e8d753ba5ad0468d30ba1189657f0';
...
assert(archivedIndex.includes(`v${LEGACY_VERSION}`), '고정 커밋의 화면 버전이 v4.19.3이 아님');
...
assert(stageScript.includes('key.startsWith(CACHE_PREFIX)'), '레거시 서비스워커 캐시 삭제 범위 격리 누락');
...
assert(stagedLegacySw.includes("const CACHE_VERSION = 'to-hwpx-legacy-v4.19.3-cache'"), '레거시 캐시 버전 패치 실패');
