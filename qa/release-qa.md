...
- [x] `?admin=1`에서만 직접 입력 탭 노출: MD/HTML/TXT/CSV/JSON 형식 버튼 선택 + 내용 붙여넣기 → 변환·다운로드 동작
- [x] 기존 호환용 `?lab=1`도 관리자 모드를 켜며 직접 입력 탭이 노출됨
- [x] 일반 접속에서는 직접 입력 탭과 관리자 모드 토글 자격이 보이지 않음
