...
- 사용자 화면과 법적 문서에 GitHub 및 기관 외부 링크가 남아 있었다.
...
- 사용자 HTML에서 GitHub/기관 링크를 제거하고 관련 회귀 검사를 추가한다.
...
원인: 실제 기관 KPI 보고서 DOCX는 XML 자체는 읽을 수 있었지만 잘못된 리터럴 속성 3개, 소수 twip 60개, 최종 `sectPr` 위치 오류, 대량의 OOXML 자식 순서 문제를 포함했다. 기존 DOCX 경로는 표 셀의 여러 문단과 수동 줄바꿈을 평문 한 문단으로 합치고 A4·앱 기본 타이포그래피를 적용해, 열리는 HWPX를 만들면서도 표 34개·4,035개 셀의 실제 내용 흐름을 크게 왜곡했다.
...
## 46. Survey 제품군 디자인·로고 벤치마킹

배경: 직접 입력 상용화는 기능 품질에 집중해 같은 제작자의 Survey 서비스와 디자인·로고 체계가 연결되지 않았다. Survey의 코드를 기준으로 색상·표면·헤더·작업 공간·심벌 구조를 대조하고, To HWPX의 문서 변환 의미에 맞게 적용한다.

자동 승인 기준:

- [x] 슬레이트 블루·차가운 업무용 배경·낮은 그림자·공통 모서리 토큰을 라이트/다크 테마에 함께 적용
- [x] 2단 헤더를 64px 단일 작업 헤더로 정리하고 로고·안내·버전 상태를 한 줄에 배치
- [x] 첫 화면을 설명/단계와 문서 선택 작업면의 2열 구조로 바꾸되 파일 선택을 가장 강한 행동으로 유지
- [x] 헤더·파비콘·PWA PNG가 같은 겹친 문서/변환 화살표 심벌을 사용
- [x] `변환 안내` 진입 시 `개발 배경` 탭과 패널이 선택된 상태인지 golden에서 검증
- [x] `npm run test:release`와 `npm run test:browsers`(Chromium·Firefox·WebKit) 통과

수동 확인 기준:

- [x] 데스크톱 1440px와 모바일 390px에서 헤더, 첫 화면, 드롭존, 보조 시작 버튼의 잘림·겹침 없음
...
## 48. 드롭존 입력 형식·아이콘 순서 정렬

자동 승인 기준:

- [x] 첫 화면 입력 안내가 `MD · PDF · DOCX · PPTX · XLSX · CSV · JSON · IPYNB` 순서와 정확히 일치
- [x] 첫 화면과 변환 작업면 드롭존이 같은 8개 아이콘을 입력 안내 순서대로 표시
- [x] CSV·JSON 전용 SVG가 자체 호스팅되고 서비스 워커 앱 셸에 포함
- [x] golden에서 안내 문구, 두 아이콘 행, 파일 선택 우선순위를 회귀 검사
- [x] `npm run test:release`와 `npm run test:browsers` 통과

수동 확인 기준:

- [x] 1100px 실제 렌더에서 8개 아이콘과 입력 안내가 한 줄로 표시되고 CSV·JSON 아이콘이 구분됨
- [ ] 실제 배포 후 데스크톱·모바일에서 아이콘 순서와 줄바꿈 확인
- [x] 푸터와 Pages 배포가 직전판 v4.21.2 커밋 `6204946`을 정확히 가리킴

## 49. 첫 화면 세로 리듬·뷰포트 밀도 개선

웹디자인 검토:

- 기존 `6vw` 상단 패딩과 `calc(100vh - 130px)` 최소 높이가 겹쳐 1147×807 화면에서 헤더와 작업 카드 사이가 약 120px 이상 벌어졌다.
- 작업 카드는 핵심 행동이므로 헤더 직후에 배치하고, 좌측 카피만 카드 중앙축에 맞춰 정보 위계를 유지한다.
- 모바일은 기존 38px 상단 여백과 세로 적층 구조를 유지해 데스크톱 수정이 작은 화면의 호흡을 훼손하지 않게 한다.

자동 승인 기준:

- [x] 데스크톱 hero 상단 여백이 28~44px, 하단 여백이 36~56px 범위
- [x] 1280×900·1147×807·1024×768에서 헤더와 작업면 간격 64px 이하
- [x] 위 데스크톱 뷰포트에서 작업면 하단이 첫 화면의 24px 안전영역 안쪽
- [x] 768×900·390×844에서 좌측 설명과 작업면이 세로 적층되고 가로 잘림 없음
- [x] `npm run test:release`와 `npm run test:browsers` 통과

수동 확인 기준:

- [x] 1280×900·1147×807·390×844 실제 렌더에서 시선이 헤더 → 핵심 설명 → 문서 선택으로 자연스럽게 흐름
- [x] 1147×807에서 작업 카드 시작점이 기존 약 224px에서 약 100px로 줄고 보조 시작 버튼까지 첫 화면에 표시
- [ ] 실제 배포 후 데스크톱·모바일에서 새 세로 리듬 확인
- [x] 푸터와 Pages 배포가 직전판 v4.21.3 커밋 `8de8a38`을 정확히 가리킴

## 50. 핵심 행동 위계·최근 작업·헤더·푸터 고도화

UX 결정:

- 파일 선택은 가장 큰 주 행동을 유지하고, 사용률이 높은 직접 입력만 메인색 보조 CTA로 승격한다.
- 최근 작업은 첫 화면에 자동 누적하지 않는다. 전용 화면에서 기록·프리셋을 관리하고, 행동 명칭은 `새 문서 변환`과 `설정 재사용`으로 분리한다.
- 지원 환경·설치는 필요할 때 찾는 저빈도 정보이므로 헤더에서 제거하되 푸터에서 기존 모달에 접근할 수 있게 유지한다.
- 검정색 다단 푸터는 콘텐츠보다 시각적으로 무거우므로 테마 표면색을 쓰는 컴팩트 2단 푸터로 축소한다. 기술 스택 나열은 오픈소스·글꼴 고지로 통합한다.

자동 승인 기준:

- [x] 직접 입력의 실제 계산 스타일이 다른 보조 버튼과 다른 메인색 배경·흰색 글자, 다크 테마 명암비 4.5:1 이상
- [x] 첫 화면에 `recent-start` DOM이 없고 최근 작업 버튼은 전용 history route로 이동
- [x] 최근 작업 화면에 `새 문서 변환` primary 버튼과 작업별 `설정 재사용` 버튼 표시
- [x] 헤더에 지원 환경 버튼이 없고 푸터의 `지원 환경·설치`로 기존 모달·키보드 포커스 동작 유지
- [x] 푸터에 `MIT License. By Sysmetrix` 표시, 기술 스택 직접 나열 제거
- [x] 1024px 이상에서 푸터 높이 190px 이하, 짧은 페이지에서는 viewport 하단 정렬
- [x] `npm run test:release`와 `npm run test:browsers` 통과

수동 확인 기준:

- [x] 1147×807·390×844 첫 화면에서 직접 입력 강조, 중립 보조 버튼, 링크 줄바꿈 확인
- [x] 1280×900 최근 작업 화면에서 새 문서 변환·설정 재사용 위계와 하단 정렬 확인
- [ ] 실제 배포 후 라이트·다크 테마에서 헤더·직접 입력·푸터 색 대비 확인
- [x] 푸터와 Pages 배포가 직전판 v4.21.4 커밋 `ee5a9ef`을 정확히 가리킴

## 51. 제이드 브랜드 분리와 직접 입력·초보자 UX 명료화

UX 결정:

- Survey와 같은 슬레이트 블루를 재사용하지 않고, 문서 변환 서비스의 독립 인상을 위해 제이드(`#146C63`)를 주색, 귤색(`#D36B32`)을 변환 포인트로 사용한다. 화면뿐 아니라 로고·파비콘·설치 아이콘·공유 이미지·manifest·법적 고지 화면까지 같은 팔레트를 쓴다.
- 문서 세부 설정은 첫 사용자에게 필요한 기본 설정보다 우선하지 않으므로 기본적으로 접어 둔다. 다만 접힌 상태에서도 `선택 사항 · 처음이라면 기본값 그대로`를 보여 주어 설정 누락 불안 없이 바로 변환할 수 있게 한다.
- 직접 입력의 TXT는 삭제하지 않는다. 회의 메모·일반 문장처럼 구조 문법이 없는 글을 Markdown·HTML·표로 오해하지 않고 빈 줄 기준 문단으로 보존하는 안전한 폴백이므로, UI 명칭을 `일반 텍스트`로 바꾼다.
- 형식 감지는 추천에 그치지 않고 JSON → CSV/TSV → HTML → Markdown → 일반 텍스트 우선순위로 실제 입력 형식을 자동 적용한다. 사용자가 형식을 누르면 수동 선택을 유지하고 `자동 감지 켜기`로만 복귀한다.

자동 승인 기준:

- [x] 라이트/다크 화면과 SVG·PNG·ICO·OG·manifest·법적 고지 화면에 제이드+귤 팔레트 적용
- [x] 푸터에서 `파일은 서버에 저장되지 않습니다.` 삭제, `© 2026 To HWPX. MIT License. By Sysmetrix` 중앙 정렬
- [x] 문서 세부 설정이 기본 접힘이며 접힌 상태에서 선택 사항·기본값 안내 노출
- [x] 직접 입력 TXT를 `일반 텍스트`로 명명하고 용도를 회의 메모·일반 문장 안전 변환으로 설명
- [x] 자동 감지·자동 적용·수동 선택 상태와 복귀 행동을 화면에 구분해 표시
- [x] 쉼표가 하나씩 든 두 줄 일반 문장은 CSV가 아니라 일반 텍스트로 판정하고, 3행 이상의 일관된 2열 데이터는 CSV로 판정
- [x] `npm run test:release` 통과
- [x] `npm run test:browsers` Chromium·Firefox·WebKit 통과

수동 확인 기준:

- [x] 1280×900 라이트·다크 첫 화면과 직접 입력·설정 화면에서 색 대비, 정렬, 잘림 없음 확인
- [x] 자동 적용 상태에서 일반 텍스트·JSON이 각각 올바른 형식 버튼과 안내로 전환됨을 확인
- [ ] 실제 배포에서 캐시를 비운 뒤 새 버전·제이드 파비콘·중앙 푸터 확인
- [ ] MD·HTML·일반 텍스트·TSV·JSON 직접 입력 결과를 한컴에서 열어 본문·표·링크 보존 확인
- [x] 푸터와 Pages 배포가 직전판 v4.21.5 커밋 `c291259`을 정확히 가리킴

## 52. 직접 입력·미리보기 균형과 양방향 스크롤 연동

UX 결정:

- 데스크톱 직접 입력 작업면은 원문과 미리보기를 1:1 폭으로 제공하고, 두 본문 영역의 높이도 같은 반응형 값(`360~520px`)을 사용한다. 개별 textarea 크기 조절로 다시 불균형해지지 않도록 데스크톱에서는 작업 높이를 한 곳에서 관리한다.
- 원문과 해석 결과는 줄 수와 렌더 높이가 다르므로 `scrollTop` 픽셀을 복사하지 않는다. 각 영역의 전체 세로 진행률을 계산해 양방향으로 맞추고, 원문 줄 번호는 원문 위치를 계속 따른다.
- 기본은 스크롤 연동 켜짐이다. 원문과 미리보기를 서로 다른 위치에서 비교해야 하는 사용자만 `스크롤 연동`을 끌 수 있으며 선택은 해당 브라우저에 저장한다.
- 모바일은 좁은 화면에서 두 영역을 계속 세로로 쌓고, 데스크톱용 고정 작업 높이를 강제하지 않는다.

자동 승인 기준:

- [x] 1100px 이상에서 편집기·미리보기 폭 차이 2px 이하, 본문 작업 높이 차이 2px 이하, 작업 높이 360px 이상
- [x] 원문 42% 스크롤 시 미리보기 진행률 오차 3% 미만
- [x] 미리보기 76% 스크롤 시 원문 진행률 오차 3% 미만이며 줄 번호도 함께 이동
- [x] 스크롤 연동 해제 시 두 영역이 독립적으로 이동
- [x] `npm run test:release` 통과
- [x] `npm run test:browsers` Chromium·Firefox·WebKit 통과

수동 확인 기준:

- [x] 1440×1000 실제 렌더에서 원문·미리보기 폭 483px, 본문 작업 높이 460px로 균형 확인
- [x] 긴 Markdown에서 양방향 스크롤 연동과 연동 해제 동작 확인
- [ ] 실제 배포에서 데스크톱·모바일 크기, 스크롤 연동, v4.21.6 직전판 링크 확인
- [x] 푸터와 Pages 배포가 직전판 v4.21.6 커밋 `b8999b4`를 정확히 가리킴
